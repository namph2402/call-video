...
// STATIC SETTINGS
// ####################################################
const urlServer = drupalSettings['video_call']['url_server'];

const socket = io(urlServer, { transports: ['websocket'] });
...
const _PEER = {
    ...
    audioOn: '<i class="bi bi-mic-fill"></i>',
    audioOff: '<i class="bi bi-mic-mute-fill red"></i>',
    videoOn: '<i class="bi bi-camera-video-fill"></i>',
    videoOff: '<i class="bi bi-camera-video-off-fill red"></i>',
    screenOn: '<i class="bi bi-display"></i>',
    screenOff: '<i class="bi bi-display red"></i>',
...
const speakerSelectDiv = getId('speakerSelectDiv');
...
let RoomURL = window.location.origin + '/meeting-room' + room_id;
let transcription;
...
function initClient() {
    ...
    if (!DetectRTC.isMobileDevice) {
        ...
        setTippy('switchPushToTalk', 'If Active, When SpaceBar keydown the microphone will be resumed, on keyup will be paused, like a walkie-talkie.', 'right',);
        setTippy('lobbyAcceptAllBtn', 'Accept', 'top');
        setTippy('lobbyRejectAllBtn', 'Reject', 'top');
        setTippy('switchBroadcasting', 'Broadcasting is the dissemination of audio or video content to a large audience (one to many)', 'right',);
        setTippy('switchLobby', 'Lobby mode lets you protect your meeting by only allowing people to enter after a formal approval by a moderator', 'right',);
        ...
        setTippy('switchHostOnlyRecording', 'Only the host (presenter) has the capability to record the meeting', 'right',);
        setTippy('switchH264Recording', 'Prioritize h.264 with AAC or h.264 with Opus codecs over VP8 with Opus or VP9 with Opus codecs', 'right',);
...
function getRoomId() {
    ...
    let roomId = queryRoomId ? queryRoomId : location.pathname.substring(14);
    if (roomId == '') roomId = makeId(12);
...
function getScreen() {
    ...
    if (screen) {
        ...
        if (queryScreen != null && (navigator.getDisplayMedia || navigator.mediaDevices.getDisplayMedia)) return queryScreen;
    }
    return false;
}
...
function isPeerPresenter() {
    ...
    if (presenter) {
        ...
        if (queryPresenter != null) return queryPresenter;
    }
    return false;
}
...
function getPeerUUID() {
    if (lS.getItemLocalStorage('peer_uuid')) return lS.getItemLocalStorage('peer_uuid');
...
function getRoomPassword() {
    ...
    if (roomPassword) {
        ...
        if (queryNoRoomPassword) roomPassword = false;
        return roomPassword;
    }
    return false;
}
    ...
    try {
        ...
        const filterUnknown = (obj) => {
            ...
            for (const [key, value] of Object.entries(obj)) {
                if (value && value !== 'Unknown') filtered[key] = value;
            }
            return filtered;
        };
...
async function whoAreYou() {
    ...
    try {
        const response = await axios.get(`${urlServer}/config`, {
            timeout: 5000,
        });
    ...
    try {
        const { data: profile } = await axios.get(`${urlServer}/profile`, { timeout: 5000 });

        if (profile) {
            const peerNamePreference = profile.peer_name || {};
            default_name = (peerNamePreference.email && profile.email) || (peerNamePreference.name && profile.name) || default_name;
    ...
    Swal.fire({
        ...
        customClass: { popup: 'init-modal-size h-auto' },
        ...
        inputValidator: (name) => {
            ...
            name = filterXSS(name);
            if (isHtml(name)) return 'Invalid name!';
            if (!getCookie(room_id + '_name')) window.localStorage.peer_name = name;
...
function handleAudio() {
    ...
    initAudioButton.className = 'bi bi-mic' + (isAudioAllowed ? '-fill' : '-mute-fill');
...
function handleVideo() {
    ...
    initVideoButton.className = 'bi bi-camera-video' + (isVideoAllowed ? '-fill' : '-off-fill');
...
async function handleAudioVideo() {
    ...
    initAudioButton.className = 'bi bi-mic' + (isAudioVideoAllowed ? '-mute-fill' : '-fill');
    initVideoButton.className = 'bi bi-camera-video' + (isAudioVideoAllowed ? '-fill' : '-off-fill');
    initAudioVideoButton.className = 'bi bi-eye' + (isAudioVideoAllowed ? '-fill' : '-slash-fill');
...
async function sound(name, force = false) {
    ...
    let sound = '/' + drupalSettings.callVideo.modulePath + 'sounds/' + name + '.wav';
...
function getParticipantsList(peers) {
    ...
    if (chatGPT) {
        ...
        li = `<li id="ChatGPT" data-to-id="ChatGPT" data-to-name="ChatGPT" class="clearfix${chatgpt_active}" onclick="rc.showPeerAboutAndMessages(this.id, 'ChatGPT', event)">
            <img  src="${image.chatgpt}" alt="avatar"/>
            <div class="about">
    ...
    li += `<li id="all" data-to-id="all" data-to-name="all" class="clearfix${public_chat_active}" onclick="rc.showPeerAboutAndMessages(this.id, 'all', event)">
            <img src="${image.all}" alt="avatar"/>
            <div class="about">
            ...
            if (isRulesActive && isPresenter) {
                ...
                >
                    ...
                    <div class="about">
                        ...
                        <div class="status"> <i class="fa fa-circle online"></i> online <i id="${peer_id}-unread-msg" class="bi bi-chat-dots-fill hidden"></i> </div>
                    </div>
