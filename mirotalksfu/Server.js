...
async function fetchAndUpdateEnv() {
    try {
        const response = await fetch(process.env.CONFIG_SERVER);
        ...
        if (data) {
            for (const [key, value] of Object.entries(data)) {
                ...
                const regex = new RegExp(`^${key.toUpperCase()}=.*`, 'm');
                const newEnvVariables = regex.test(envVariables) ? envVariables.replace(regex, `${key.toUpperCase()}=${value}`) : `${envVariables}\n${key.toUpperCase()}=${value}`;
    ...
    */
    ...
    function startServer() {
        ...
        */
        ...
        // api meeting room endpoint
        app.post([restApi.basePath + '/api-meeting'], (req, res) => {
            // Check if endpoint allowed
            if (restApi.allowed && !restApi.allowed.meeting) {
                return res.status(403).json({
                    error: 'This endpoint has been disabled. Please contact the administrator for further information.',
            ...
            // setup meeting ID
            const meetingID = api.getMeetingRoomID();
            res.json({ meeting: meetingID });
        });

        // api join room endpoint
        app.post([restApi.basePath + '/api-join'], (req, res) => {
            // Check if endpoint allowed
            if (restApi.allowed && !restApi.allowed.join) {
                return res.status(403).json({
                    error: 'This endpoint has been disabled. Please contact the administrator for further information.',
            ...
            // setup Join URL
            if(!roomList.has(req.body.room)) {
                return res.status(404).json({
                    error: 'Not found room',
                });
            }

            res.json({ join: req.body.room });
        });
